...
## Project Files and Turn-in

Like previous projects, fork the following project repository to your Github account:

*   <https://github.com/psosera/blockchain>

Make sure to push any commits you make to your forked copy of the repository on Github.
When you are done, use the "upload Github repository" option on Gradescope to submit your work!
...
* [MessageDigest class](https://docs.oracle.com/javase/8/docs/api/java/security/MessageDigest.html)
...
Rather than expose a constructor, `MessageDigest` provides a static *factory method* that creates instances of the class for you.
`getInstance` takes a single argument which is the hash algorithm to be employed by the object.
...
For this project, we place the following restriction on our hashes:
...
Cryptographic hashing tells us that the only method of mining is:
...
Returning to our example, suppose that we add another block to the chain after adding block 0:
...
Furthermore, because of the architecture of the blockchain, it is sufficient to check only the *final block's hash* of all the replicas to determine the majority, which is efficient to do!
...
* [`Byte.toUnsignedInt`](https://docs.oracle.com/javase/8/docs/api/java/lang/Byte.html#toUnsignedInt-byte-)
...
2. If so, it should *cast* `other` to type `Hash`, *e.g.*, `Hash o = (Hash) other` and then use the `Arrays.equals` [static method](https://docs.oracle.com/javase/8/docs/api/java/util/Arrays.html#equals-byte:A-byte:A-) to perform the appropriate equality check on the two `Hash` object's arrays.
...
To convert integers and longs into byte arrays, use the [`ByteBuffer` class](https://docs.oracle.com/javase/8/docs/api/java/nio/ByteBuffer.html) which allows you to store data types as arrays of bytes.
...
This is to enable a style of method called "method chaining".
For example, here is an example of producing a byte array from an integer `x`:
...
Furthermore, you should also search the space of nonces by searching **all possible non-negative `long` values in increasing order starting at zero**.

### The BlockChain Class

Use `Block` to implement a `BlockChain` class which is a singly linked list with a `first` and `last` pointer (*i.e.*, pointer to the first and last elements in the list).
`BlockChain` should have a nested `Node` class in the standard style of linked list implementation in Java.
...
* `Block mine(int amount)`: mines a new candidate block to be added to the list.
...
This mimics more closely how Bitcoin works in real-life: as miners race to discover blocks, they may collectively discover several nonces that work.
...
You can do so using the [Scanner class](https://docs.oracle.com/en/java/javase/23/docs/api/java.base/java/util/Scanner.html) and its associated `next*` methods.
